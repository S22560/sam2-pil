...
parser = argparse.ArgumentParser(description="画像ディレクトリと出力ディレクトリを指定するスクリプト")

# 引数の設定
parser.add_argument("--image_dir", type=str, default="../isa_scene", 
                    help="入力画像が保存されているディレクトリのパス")
parser.add_argument("--output_dir", type=str, default="../isa_scene_char", 
                    help="処理後の結果を保存するディレクトリのパス")
...
def sam2inference(images,image_paths,boxes):
    with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
        ...
        for i, image in enumerate(image_paths[len(images)-len(video_segments):]):
            current_frame = np.array(Image.open(image))
            # ビデオフレームごとのマスクを取得
...
sorted_folder_list = sorted(
    folder_list,
    key=lambda folder: int(folder.replace(".mp4", "").replace("scene_", ""))
)

for folder in sorted_folder_list:
    if last_generated_data is not None:
        scene_id = int(folder.replace(".mp4","").replace("scene_",""))
        if scene_id<=last_generated_data:
            continue
    # if folder>="scene_178":
    #     continue
    print(folder)
    try:
        ...
        video_filepath = os.path.join(output_dir,f'{folder}.mp4')

        # output_path = os.path.join(output_dir,folder)

        images_raw = sorted(glob.glob(f"{image_path}/*.jpg"))[1:]
        images = [images_raw[0],images_raw[0]] + images_raw
        if os.path.exists(video_filepath) or len(images_raw)<16:
            continue
        for ann_frame_idx in range(len(images)):
            persons = detect_person(images[ann_frame_idx])
            if len(persons)!=0:
                break
        if len(persons)==0:
            continue
        boxes = np.stack([i[0] for i in persons])
        images_pil = [Image.open(i) for i in images]
